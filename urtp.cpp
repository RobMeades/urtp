 ...
 * be unusedValue for each orientation (where unusedValue can mean all
 * 0xFF or all 0).
 */
static const int byteUnused[4][5] = {{2, 4, 5, 6, 7},
                                     {0, 1, 4, 6, 7},
                                     {0, 1, 2, 3, 6},
                                     {0, 2, 3, 4, 5}};

/** The meaning of unused.
 */
static char unusedValue = 0xFF;
...
int Urtp::getRotation(const uint32_t *stereoSample)
{
    ...
    for (x = 0; x < sizeof(_rotationBuffer) / sizeof(_rotationBuffer[0]); x++) {
        ...
        // Find the pattern of unusedValue bytes
        for (y = 0; (y < sizeof(byteUnused) / sizeof(byteUnused[0])) && !foundIt; y++) {
            foundIt = true;
            for (unsigned int z = 0; (z < sizeof(byteUnused[0]) / sizeof(byteUnused[0][0])) && foundIt; z++) {
                if (*(pByte + byteUnused[y][z]) != unusedValue) {
                    foundIt = false;
                }
            }
            if (foundIt) {
                // If we have found a pattern of unusedValue bytes that match, it could
                // be a fluke, so check if the LSB of the used bytes is not also
                // unusedValue (the upper bytes could well be unusedValue if the
                // number is a small negative number)
                if (*(pByte + byteSelect[y][0]) == unusedValue) {
                    foundIt = false;
                }
            }
        }
    ...
    // If we did not find the rotation result on this occasion,
    // it might be because the unusedValue is 0 rather than 0xFF
    // (or vice-versa), so toggle the bits in unusedValue so that we
    // have a chance of succeeding next time.
    if (rotationResult < 0) {
